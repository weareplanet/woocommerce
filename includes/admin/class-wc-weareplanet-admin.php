...
class WC_WeArePlanet_Admin {
	...
	public function handle_subscription_plugin_active() {
		...
		if ( ! WooCommerce_WeArePlanet::is_subscriptions_compatible() ) {
			return;
		}

		$plugin_name = 'woocommerce-weareplanet-subscription.php';
		$subscriptions_legacy_plugin = WooCommerce_WeArePlanet::find_plugin_path( $plugin_name );
		...
		if ( $gateway instanceof WC_WeArePlanet_Gateway ) {
			...
			if ( $transaction_info->get_state() === \WeArePlanet\Sdk\Model\TransactionState::AUTHORIZED ) {
				if ( WC_WeArePlanet_Entity_Completion_Job::count_running_completion_for_transaction(
					...
					echo '<button type="button" class="button button-primary weareplanet-completion-show">' . esc_html__( 'Completion', 'woo-weareplanet' ) . '</button>';
				}
			}
		}
	}
	...
	public function enque_script_and_css() {
		...
		if ( 'shop_order' == $post_type ) {
			...
			$localize = array(
				'i18n_do_void'  => esc_html__( 'Are you sure you wish to process this void? This action cannot be undone.', 'woo-weareplanet' ),
				'i18n_do_completion' => esc_html__( 'Are you sure you wish to process this completion? This action cannot be undone.', 'woo-weareplanet' ),
			);
	...
	public function plugin_action_links( $links ) {
		$action_links = array(
			'settings' => '<a href="' . admin_url( 'admin.php?page=wc-settings&tab=weareplanet' ) . '" aria-label="' .
					esc_html__( 'View Settings', 'woo-weareplanet' ) . '">' . esc_html__( 'Settings', 'woo-weareplanet' ) . '</a>',
		);
