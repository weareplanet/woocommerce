...
class WC_WeArePlanet_Admin_Settings_Page extends WC_Settings_Page {
	...
	public function get_default_settings() {
		$settings = array(
			array(
				'links' => array(
					'https://plugin-documentation.weareplanet.com/weareplanet/woocommerce/3.4.1/docs/en/documentation.html' => esc_html__( 'Documentation', 'woo-weareplanet' ),
            ...
            array(
                'title'   => esc_html__( 'Disable Pending Email', 'woo-weareplanet' ),
                'desc'    => esc_html__( 'Enable this setting to prevent WooCommerce from sending "pending payment" emails for WeArePlanet payment methods.', 'woo-weareplanet' ),
                'id'      => WooCommerce_WeArePlanet::WEAREPLANET_CK_DISABLE_PENDING_EMAIL,
                'type'    => 'checkbox',
                'default' => 'no',
                'css'     => 'min-width:300px;',
            ),
			...
			array(
				...
				'desc_tip' => esc_html__( 'This field allows you to apply custom styling to the payment form and payment page. The styling is defined in your Space settings in the Portal.', 'woo-weareplanet' ),
	...
	public function get_order_status_settings() {
		$settings = [];
		$is_custom_status_mapping_enabled = WC_WeArePlanet_Helper::is_custom_status_mapping_enabled();

		$settings[] = array(
			'title' => esc_html__( 'Order Status Settings', 'woo-weareplanet' ),
			'type' => 'title',
			'id' => 'order_status_mapping_options',
			'desc' => esc_html__( 'Control how WooCommerce order statuses are mapped to WeArePlanet transaction states.', 'woo-weareplanet' ),
		);

		$settings[] = array(
			'title' => esc_html__( 'Enable Custom Status Mapping', 'woo-weareplanet' ),
			'desc' => esc_html__( 'If enabled, custom statuses (such as Manual Decision) will be used instead of the default WooCommerce statuses.', 'woo-weareplanet' ),
			'id' => WooCommerce_WeArePlanet::WEAREPLANET_CK_ENABLE_CUSTOM_STATUS_MAPPING,
			'type'    => 'checkbox',
			'default' => 'no',
			'css'     => 'min-width:300px;',
		);
		...
		if ( $is_custom_status_mapping_enabled ) {
			$settings[] = array(
				'title' => esc_html__( 'Status Mappings', 'woo-weareplanet' ),
				'type' => 'title',
				'id' => 'order_status_mappings',
				'desc' => esc_html__( 'Choose which WooCommerce order status should be used for each WeArePlanet transaction status.', 'woo-weareplanet' ),
			);
			...
			$settings[] = array(
				...
				'desc'  => sprintf(
					'<div class="weareplanet-status-mapping-header">
						<span class="weareplanet-status-mapping-col-left">%s</span>
						<span class="weareplanet-status-mapping-col-right">%s</span>
					</div>'
					,
					esc_html__( 'WeArePlanet payment status', 'woo-weareplanet' ),
					esc_html__( 'WooCommerce Order Status', 'woo-weareplanet' )
				),
			);
			...
			foreach ( $weareplanet_statuses as $status_key => $status_label ) {
				...
				$settings[] = array(
					...
					'desc' => sprintf(
						...
						__( 'Set a custom WooCommerce order status to be applied automatically when a transaction is in the %s state.', 'woo-weareplanet' ),
