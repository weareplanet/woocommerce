...
class WC_WeArePlanet_Admin_Refund {
	...
	public static function execute_refund( WC_Order $order, WC_Order_Refund $refund ) {
		...
		try {
			...
			if ( ! $transaction_info->get_id() ) {
				throw new Exception( esc_html__( 'Could not load corresponding transaction', 'woo-weareplanet' ) );
			}
			...
			if ( WC_WeArePlanet_Entity_Refund_Job::count_running_refund_for_transaction(
				...
				throw new Exception( esc_html__( 'Please wait until the pending refund is processed.', 'woo-weareplanet' ) );
			}
	...
	protected static function send_refund( $refund_job_id ) {
		...
		if ( $refund_job->get_state() != WC_WeArePlanet_Entity_Refund_Job::WEAREPLANET_STATE_CREATED ) {
		...
		try {
			...
			$refund_job->set_state( WC_WeArePlanet_Entity_Refund_Job::WEAREPLANET_STATE_SENT );

			if ( $executed_refund->getState() == \WeArePlanet\Sdk\Model\RefundState::PENDING ) {
				$refund_job->set_state( WC_WeArePlanet_Entity_Refund_Job::WEAREPLANET_STATE_PENDING );
			}
			$refund_job->save();
			WC_WeArePlanet_Helper::instance()->commit_database_transaction();
			...
			if ( $e->getResponseObject() instanceof \WeArePlanet\Sdk\Model\ClientError ) {
				...
				$refund_job->save();
				WC_WeArePlanet_Helper::instance()->commit_database_transaction();
				WooCommerce_WeArePlanet::instance()->log( 'Error sending refund. ' . $error_message, WC_Log_Levels::INFO );
				/* translators: %s: message */
				throw new Exception( sprintf( esc_html__( 'There has been an error while sending the refund to the gateway. Error: %s', 'woo-weareplanet' ), esc_html( $error_message ) ) );
			}
		} catch ( Exception $e ) {
			$refund_job->save();
			$error_message = $e->getMessage();
	...
	public static function update_refunds() {
		...
		foreach ( $to_process as $id ) {
			try {
				...
				$message = sprintf( esc_html__( 'Error updating refund job with id %1$d: %2$s', 'woo-weareplanet' ), $id, $e->getMessage() );
	...
	private static function create_refund_job( WC_Order $order, WC_Order_Refund $refund, \WeArePlanet\Sdk\Model\RefundCreate $refund_create ) {
		...
		$refund_job->set_state( WC_WeArePlanet_Entity_Refund_Job::WEAREPLANET_STATE_CREATED );
