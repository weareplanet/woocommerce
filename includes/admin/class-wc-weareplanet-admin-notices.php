...
class WC_WeArePlanet_Admin_Notices {
	...
	public static function blocks_warning() {
		...
		if ($is_blocks_active || ($is_wc_active && $is_wc_version_gt)) {	
			...
			$message = __( 'The WeArePlanet plugin is currently incompatible with the new Woocommerce Blocks Checkout Template (bundled with Woocommerce 8.0.0+). We are working on compatibility. If payment methods do not appear for you when using WeArePlanet with checkout blocks, please replace the checkout page block with the shortcode for the checkout: <pre>[woocommerce_checkout]</pre>', 'sample-text-domain' );
