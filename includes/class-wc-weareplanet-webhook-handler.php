...
class WC_WeArePlanet_Webhook_Handler {
	...
	public static function process() {
		...
		try {
			...
			$request = new WC_WeArePlanet_Webhook_Request( json_decode( $clean_data ) );
			$client = WC_WeArePlanet_Helper::instance()->get_api_client();
			...
			if ( ! empty( $signature ) && $client->getWebhookEncryptionService()->isContentValid( $signature, $clean_data ) ) {
				WC_WeArePlanet_Webhook_Strategy_Manager::instance()->process( $request );
			}
