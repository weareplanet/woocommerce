 ...
 */
final class WC_WeArePlanet_Blocks_Support extends AbstractPaymentMethodType {
	/**
	 * Payment method name defined by payment methods extending this class.
	 *
	 * @var string
	 */
	protected $name = 'weareplanet';

	/**
	 * Initializes the payment method type.
	 */
	public function initialize() {
		$this->settings['space_id'] = get_option( WooCommerce_WeArePlanet::WEAREPLANET_CK_SPACE_ID );
	}
	...
	public function get_payment_method_script_handles() {
		...
		wp_register_script(
			'WooCommerce_WeArePlanet_blocks_support',
			WooCommerce_WeArePlanet::instance()->plugin_url() . '/assets/js/frontend/blocks/build/index.js',
			$dependencies,
			$version,
			true
		);
		...
		wp_localize_script(
			'WooCommerce_WeArePlanet_blocks_support',
			'weareplanet_block_params',
			array(
				'weareplanet_nonce' => wp_create_nonce( 'weareplanet_nonce_block' ),
			)
		);

		return array( 'WooCommerce_WeArePlanet_blocks_support' );
	}
	...
	public function get_payment_method_data() {
		return array(
			'title' => 'WeArePlanet',
			'description' => 'WeArePlanet description',
		);
	}
	...
	public static function get_payment_methods(): array {
		try {
			...
			$available_payment_methods = WC_WeArePlanet_Service_Transaction::instance()->get_possible_payment_methods_for_cart();

			$payment_plugin = array_filter(
			  $payment_gateways,
			  fn( $key ) => str_contains( $key, 'weareplanet_' ),
			...
			$payments_list =
			  array_map(
				function ( $payment_gateway ) use ( $available_payment_methods ) {
					$has_subscription = WC_WeArePlanet_Zero_Gateway::cart_has_subscription();
					$cartTotal = (WC()->cart && WC()->cart->total) ?? 0;

					$isPaymentMethodVisibleOnCheckout = $payment_gateway->get_payment_configuration_id() === WC_WeArePlanet_Zero_Gateway::ZERO_PAYMENT_CONF_ID && $cartTotal == 0;
					$integration_mode = $payment_gateway->get_payment_configuration_id() === WC_WeArePlanet_Zero_Gateway::ZERO_PAYMENT_CONF_ID
						? WC_WeArePlanet_Integration::WEAREPLANET_PAYMENTPAGE
						: get_option( WooCommerce_WeArePlanet::WEAREPLANET_CK_INTEGRATION );
	...
	public static function get_payment_methods_json() {
		if ( ! isset( $_POST['weareplanet_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['weareplanet_nonce'] ) ), 'weareplanet_nonce_block' ) ) {  //phpcs:ignore
	...
	public static function enqueue_portal_scripts() {
		...
		try {
			...
			switch( get_option( WooCommerce_WeArePlanet::WEAREPLANET_CK_INTEGRATION ) ) {
				case WC_WeArePlanet_Integration::WEAREPLANET_IFRAME:
					$js_url = $transaction_service->get_javascript_url_for_transaction( $transaction );
					break;
				case WC_WeArePlanet_Integration::WEAREPLANET_LIGHTBOX:
	...
	public static function process_payment( PaymentContext $context, PaymentResult &$result ) {
		...
		if ( ! $payment_method_object instanceof \WC_WeArePlanet_Gateway ) {
			return;
		}
		...
		$transaction_service->api_client->addDefaultHeader(
			...
			WC_WeArePlanet_Helper::WEAREPLANET_CHECKOUT_TYPE_BLOCKS
		);
