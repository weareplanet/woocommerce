...
final class WC_WeArePlanet_Blocks_Support extends AbstractPaymentMethodType {
	...
	public static function get_payment_methods() {
		...
		$paymentMethods = WC()->session->get( 'weareplanet_payments_list' );
		if ($paymentMethods) {
			return wp_send_json( array_values( $paymentMethods ) );
		}
		if ( ! isset( $_POST['weareplanet_nonce'] ) || ! wp_verify_nonce( $_POST['weareplanet_nonce'], 'weareplanet_nonce_block' ) ) {  //phpcs:ignore
			wp_send_json_error( 'Invalid request', 403 );
		}
		$payment_gateways = WC()->payment_gateways()->payment_gateways();

		// From all the payment gateways, only use the ones provided by this module.
		$payment_plugin = array_filter( $payment_gateways, fn( $key ) => str_contains( $key, 'weareplanet_' ), ARRAY_FILTER_USE_KEY );

		// Build the list with the keys expected by WooCommerce Blocks registering functionality.
		$payments_list = array_map(
			function ( WC_WeArePlanet_Gateway $payment_gateway ) {
				return array(
					...
					'integration_mode' => ( get_option( WooCommerce_WeArePlanet::WEAREPLANET_CK_INTEGRATION ) == WC_WeArePlanet_Integration::WEAREPLANET_LIGHTBOX ) ? 'lightbox' : 'iframe',
		...
		WC()->session->set( 'weareplanet_payments_list', $payments_list );
	...
	public static function enqueue_portal_scripts() {
		...
		try {
			...
			if ( get_option( WooCommerce_WeArePlanet::WEAREPLANET_CK_INTEGRATION ) == WC_WeArePlanet_Integration::WEAREPLANET_LIGHTBOX ) {
