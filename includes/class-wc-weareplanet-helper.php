...
class WC_WeArePlanet_Helper {
	...
	public function get_api_client() {
		if ( null === $this->api_client ) {
			...
			if ( ! empty( $user_id ) && ! empty( $user_key ) ) {
				...
				throw new Exception( __( 'The API access data is incomplete.', 'woo-weareplanet' ) );
			}
		}
	...
	public function translate( $translated_string, $language = null ) {
		...
		try {
			/* @var WC_WeArePlanet_Provider_Language $language_provider */
	...
	public function get_currency_fraction_digits( $currency_code ) {
		/* @var WC_WeArePlanet_Provider_Currency $currency_provider */
	...
	public function get_total_amount_including_tax( array $line_items, bool $excludeDiscounts = false) {
		...
		foreach ( $line_items as $line_item ) {
			...
			if ( $excludeDiscounts && $type === \WeArePlanet\Sdk\Model\LineItemType::DISCOUNT && strpos( $name, WC_WeArePlanet_Packages_Coupon_Discount::COUPON ) !== false ) {
	...
	public function cleanup_line_items( array $line_items, $expected_sum, $currency ) {
		...
		$has_coupons = apply_filters( 'wc_weareplanet_packages_coupon_cart_has_coupon_discounts_applied', $currency );
		...
		if ( $has_coupons ) {
			$result = apply_filters( 'wc_weareplanet_packages_coupon_process_line_items_with_coupons', $line_items, $expected_sum, $currency );
		...
		if ( 0 != $inconsistent_amount ) {
			$enforce_consistency = get_option( WooCommerce_WeArePlanet::CK_ENFORCE_CONSISTENCY );
			switch ( $enforce_consistency ) {
				case 'no':
					...
					$line_item->setName( __( 'Adjustment', 'woo-weareplanet' ) );
					...
					break;
				default:
					throw new WC_WeArePlanet_Exception_Invalid_Transaction_Amount( $effective_sum, $rounded_expected_sum );
			}
		}
	...
	public function lock_by_transaction_id( $space_id, $transaction_id ) {
		...
		$wpdb->query( $wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'wc_weareplanet_transaction_info WHERE transaction_id = %d and space_id = %d FOR UPDATE', $transaction_id, $space_id ) );

		$wpdb->update(
			$wpdb->prefix . 'wc_weareplanet_transaction_info',
			$data_array,
			array(
	...
	public function try_to_parse_date( $date_string ) {
		$date_of_birth = false;
		$custom_date_of_birth_format = apply_filters( 'wc_weareplanet_custom_date_of_birth_format', '' );
	...
	protected function restock_items_for_order( WC_Order $order ) {
		if ( 'yes' === get_option( 'woocommerce_manage_stock' ) && $order && apply_filters( 'wc_weareplanet_can_increase_order_stock', true, $order ) && count( $order->get_items() ) > 0 ) {
			foreach ( $order->get_items() as $item ) {
				...
				if ( $item->is_type( 'line_item' ) && $product && $product->managing_stock() ) {
					...
					if ( ! is_wp_error( $new_stock ) ) {
						...
						$order->add_order_note( sprintf( __( '%1$s stock increased from %2$s to %3$s.', 'woo-weareplanet' ), $item_name, $new_stock - $qty, $new_stock ) );
					}
				}
			}
