...
/**
 *
 * WC_WeArePlanet_Service_Refund Class
 *
 * WeArePlanet
 ...
 */
class WC_WeArePlanet_Service_Refund extends WC_WeArePlanet_Service_Abstract {

	/**
	 * The refund API service.
	 *
	 * @var \WeArePlanet\Sdk\Service\RefundService refund service.
	 */
	...
	public function create( WC_Order $order, WC_Order_Refund $refund ) {
		...
		$transaction = WC_WeArePlanet_Service_Transaction::instance()->get_transaction( $transaction_info->get_space_id(), $transaction_info->get_transaction_id() );
	 ...
	 * @param \WeArePlanet\Sdk\Model\LineItemReductionCreate[] $reductions reductions.
	 * @return \WeArePlanet\Sdk\Model\LineItemReductionCreate[]
	 * @throws Exception Exception.
	 */
	protected function fix_reductions( WC_Order_Refund $refund, \WeArePlanet\Sdk\Model\Transaction $transaction, array $reductions ) {
	 ...
	 * @param \WeArePlanet\Sdk\Model\LineItem[]                $base_line_items base line items.
	...
	private function distribute_rounding_difference( array $reductions, $index, $remainder, array $base_line_items,
		...
		if ( 0 != $new_remainder && $index + 1 < count( $reductions ) ) {
			...
			if ( $new_remainder > pow( 0.1, $digits + 1 ) ) {
				throw new Exception( __( 'Could not distribute the rounding difference.', 'woo-weareplanet' ) );
			} else {
	...
	protected function get_reductions( WC_Order $order, WC_Order_Refund $refund ) {
		...
		foreach ( $refund->get_items() as $item ) {
			...
			$unique_id = $order_item->get_meta( '_weareplanet_unique_line_item_id', true );
		...
		foreach ( $refund->get_fees() as $fee ) {
			...
			$unique_id = $order_fee->get_meta( '_weareplanet_unique_line_item_id', true );
		...
		foreach ( $refund->get_shipping_methods() as $shipping ) {
			...
			$unique_id = $order_shipping->get_meta( '_weareplanet_unique_line_item_id', true );
	 ...
	 *
	 * @param int                                           $space_id space id.
	 * @param \WeArePlanet\Sdk\Model\RefundCreate $refund refund.
	 * @return \WeArePlanet\Sdk\Model\Refund
	 * @throws Exception Exception.
	 */
	public function refund( $space_id, \WeArePlanet\Sdk\Model\RefundCreate $refund ) {
	 ...
	 * @throws Exception Exception.
	 */
	protected function get_base_line_items( \WeArePlanet\Sdk\Model\Transaction $transaction, \WeArePlanet\Sdk\Model\Refund $refund = null ) {
	 ...
	 *
	 * @param \WeArePlanet\Sdk\Model\Transaction $transaction transaction.
	 * @return \WeArePlanet\Sdk\Model\TransactionInvoice transaction invoice.
	 * @throws \WeArePlanet\Sdk\ApiException ApiException.
	 */
	protected function get_transaction_invoice( \WeArePlanet\Sdk\Model\Transaction $transaction ) {
		$query = new \WeArePlanet\Sdk\Model\EntityQuery();
		...
		$invoice_service = new \WeArePlanet\Sdk\Service\TransactionInvoiceService( WC_WeArePlanet_Helper::instance()->get_api_client() );
	 ...
	 *
	 * @param \WeArePlanet\Sdk\Model\Transaction $transaction transaction.
	 * @param \WeArePlanet\Sdk\Model\Refund|null $refund refund.
	 * @return false|\WeArePlanet\Sdk\Model\Refund
	 * @throws \WeArePlanet\Sdk\ApiException ApiException.
	 */
	protected function get_last_successful_refund( \WeArePlanet\Sdk\Model\Transaction $transaction, \WeArePlanet\Sdk\Model\Refund $refund = null ) {
		$query = new \WeArePlanet\Sdk\Model\EntityQuery();

		$filter = new \WeArePlanet\Sdk\Model\EntityQueryFilter();
		$filter->setType( \WeArePlanet\Sdk\Model\EntityQueryFilterType::_AND );
		$filters = array(
			$this->create_entity_filter( 'state', \WeArePlanet\Sdk\Model\RefundState::SUCCESSFUL ),
			$this->create_entity_filter( 'transaction.id', $transaction->getId() ),
		);
		if ( null != $refund ) {
			$filters[] = $this->create_entity_filter( 'id', $refund->getId(), \WeArePlanet\Sdk\Model\CriteriaOperator::NOT_EQUALS );
		}
		...
		$query->setOrderBys(
			array(
				$this->create_entity_order_by( 'createdOn', \WeArePlanet\Sdk\Model\EntityQueryOrderByType::DESC ),
			)
		);
	...
	protected function get_refund_service() {
		if ( null == $this->refund_service ) {
			$this->refund_service = new \WeArePlanet\Sdk\Service\RefundService( WC_WeArePlanet_Helper::instance()->get_api_client() );
		}
