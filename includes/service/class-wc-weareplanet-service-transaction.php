...
/**
 *
 * WC_WeArePlanet_Service_Transaction Class
 *
 * WeArePlanet
...
class WC_WeArePlanet_Service_Transaction extends WC_WeArePlanet_Service_Abstract {
	...
	public function update_transaction_info( \WeArePlanet\Sdk\Model\Transaction $transaction, WC_Order $order ) {
		...
		$info = apply_filters( 'wc_weareplanet_update_transaction_info', $info, $transaction, $order );
		$info->save();
		return $info;
	}
	...
	private function get_possible_payment_methods( $transaction_source ) {
	    ...
	    if ( ! isset( self::$possible_payment_method_cache[ $id ] ) || is_null( self::$possible_payment_method_cache[ $id ] ) ) {
	        try {
	            ...
	            $integration_method = get_option( WooCommerce_WeArePlanet::CK_INTEGRATION );
	...
	public function confirm_transaction( $transaction_id, $space_id, WC_Order $order, $method_configuration_id ) {
		...
		for ( $i = 0; $i < 5; $i++ ) {
			try {
				...
				if ( $transaction->getState() != \WeArePlanet\Sdk\Model\TransactionState::PENDING ) {
					throw new Exception( __( 'The checkout expired, please try again.', 'woo-weareplanet' ) );
				}
				...
				$pending_transaction = apply_filters( 'wc_weareplanet_modify_confirm_transaction', $pending_transaction, $order );
	...
	protected function getOrderReference( $order ) {
		$reference_type = get_option( WooCommerce_WeArePlanet::CK_ORDER_REFERENCE );

		if ( WC_WeArePlanet_Order_Reference::ORDER_NUMBER == $reference_type ) {
	...
	protected function get_order_billing_address( WC_order $order ) {
		...
		$custom_billing_date_of_birth_meta_name = apply_filters( 'wc_weareplanet_billing_date_of_birth_order_meta_name', '' );
		...
		$gender_string = '';
		$custom_billing_gender_meta_name = apply_filters( 'wc_weareplanet_billing_gender_order_meta_name', '' );
		...
		return apply_filters( 'wc_weareplanet_modify_order_billing_address', $address, $order );
	}
	...
	protected function get_order_shipping_address( WC_order $order ) {
		...
		$date_of_birth_string = '';
		$custom_shipping_date_of_birth_meta_name = apply_filters( 'wc_weareplanet_shipping_date_of_birth_order_meta_name', '' );
		...
		$gender_string = '';
		$custom_shipping_gender_meta_name = apply_filters( 'wc_weareplanet_shipping_gender_order_meta_name', '' );
		...
		return apply_filters( 'wc_weareplanet_modify_order_shipping_address', $address, $order );
	...
	public function get_transaction_from_session() {
		...
		if ( ! isset( self::$transaction_cache[ $current_cart_id ] ) || null == self::$transaction_cache[ $current_cart_id ] ) {
			...
			$configured_space_id = get_option( WooCommerce_WeArePlanet::CK_SPACE_ID );
	...
	protected function create_transaction_by_order( WC_Order $order ) {
		...
		$space_view_id = get_option( WooCommerce_WeArePlanet::CK_SPACE_VIEW_ID, null );
		...
		$create_transaction = apply_filters( 'wc_weareplanet_modify_order_create_transaction', $create_transaction, $order );
	...
	protected function create_transaction_from_session() {
		...
		$create_transaction = apply_filters( 'wc_weareplanet_modify_session_create_transaction', $create_transaction );
		$transaction = $this->get_transaction_service()->create( $space_id, $create_transaction );
		$this->store_transaction_ids_in_session( $transaction );
		return $transaction;
	}
	...
	protected function load_and_update_transaction_for_order( WC_Order $order, WC_WeArePlanet_Entity_Transaction_Info $existing_transaction ) {
		...
		for ( $i = 0; $i < 5; $i++ ) {
			try {
				...
				$pending_transaction->setVersion( $transaction->getVersion() );
				$this->assemble_order_transaction_data( $order, $pending_transaction );
				$pending_transaction = apply_filters( 'wc_weareplanet_modify_order_pending_transaction', $pending_transaction, $order );
	...
	protected function load_and_update_transaction_from_session() {
		...
		for ( $i = 0; $i < 5; $i++ ) {
			try {
				...
				if ( $transaction->getState() != \WeArePlanet\Sdk\Model\TransactionState::PENDING || ( ! empty( $transaction->getCustomerId() ) && $transaction->getCustomerId() != $this->get_customer_id() ) ) {
				...
				$pending_transaction = apply_filters( 'wc_weareplanet_modify_session_pending_transaction', $pending_transaction );
				return $this->get_transaction_service()->update( $space_id, $pending_transaction );
			} catch ( \Exception $e ) {
				$last = $e;
			}
	...
	protected function get_session_billing_address() {
		...
		return apply_filters( 'wc_weareplanet_modify_session_billing_address', $address );
	}
	...
	protected function get_session_shipping_address() {
		...
		return apply_filters( 'wc_weareplanet_modify_session_shipping_address', $address );
	}
