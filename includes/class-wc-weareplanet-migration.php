...
class WC_WeArePlanet_Migration {
	...
	private static function check_requirements() {
		...
		if ( version_compare( PHP_VERSION, WC_WEAREPLANET_REQUIRED_PHP_VERSION, '<' ) ) {
			$errors[] = sprintf( esc_html__( "PHP %1\$s+ is required. (You're running version %2\$s)", 'woo-weareplanet' ), WC_WEAREPLANET_REQUIRED_PHP_VERSION, PHP_VERSION );
		}
		if ( version_compare( $wp_version, WC_WEAREPLANET_REQUIRED_WP_VERSION, '<' ) ) {
			$errors[] = sprintf( esc_html__( "WordPress %1\$s+ is required. (You're running version %2\$s)", 'woo-weareplanet' ), WC_WEAREPLANET_REQUIRED_WP_VERSION, $wp_version );

		}

		if ( ! is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
			/* translators: %s+: version */
			$errors[] = sprintf( esc_html__( 'Woocommerce %s+ has to be active.', 'woo-weareplanet' ), WC_WEAREPLANET_REQUIRED_WC_VERSION );
		} else {
			...
			if ( version_compare( $woocommerce_data['Version'], WC_WEAREPLANET_REQUIRED_WC_VERSION, '<' ) ) {
				$errors[] = sprintf( esc_html__( "Woocommerce %1\$s+ is required. (You're running version %2\$s)", 'woo-weareplanet' ), WC_WEAREPLANET_REQUIRED_WC_VERSION, $woocommerce_data['Version'] );
			}
		}
		...
		try {
			...
			$errors[] = __( "Install the PHP cUrl extension or ensure the 'stream_socket_client' function is available.", 'woo-weareplanet' );
		}
		...
		if ( ! empty( $errors ) ) {
			...
			wp_die(
				sprintf(
					...
					esc_html__( 'Please check the following requirements before activating: %s', 'woo-weareplanet' ),
					esc_html( $error_list )
				),
				esc_html__( 'Could not activate plugin WeArePlanet.', 'woo-weareplanet' ),
	...
	public static function wp_initialize_site( WP_Site $site, array $args ) { //phpcs:ignore
		if ( is_plugin_active_for_network( 'woo-weareplanet/woocommerce-weareplanet.php' ) ) {
			...
			// Defensive check: blog_id should always be set, otherwise will be shown as weareplanet error
			if ( ! $blog_id ) {
				$logger = wc_get_logger();
				$logger->error( 'Could not detect blog_id during wp_initialize_site.', [ 'source' => 'weareplanet' ] );
				return;
			}
	...
	private static function migrate_db() {
		$current_version = get_option( self::WEAREPLANET_CK_DB_VERSION, 0 );
		foreach ( self::$db_migrations as $version => $function_name ) {
			if ( version_compare( $current_version, $version, '<' ) ) {
				...
				update_option( self::WEAREPLANET_CK_DB_VERSION, $version );
	...
	public static function wpmu_drop_tables( $tables ) {
		...
		$tables[] = $wpdb->prefix . 'weareplanet_method_config';
		$tables[] = $wpdb->prefix . 'weareplanet_transaction_info';
		$tables[] = $wpdb->prefix . 'weareplanet_token_info';
		$tables[] = $wpdb->prefix . 'weareplanet_completion_job';
		$tables[] = $wpdb->prefix . 'weareplanet_void_job';
		$tables[] = $wpdb->prefix . 'weareplanet_refund_job';
	...
	public static function plugin_row_meta( $links, $file ) {
		if ( WC_WEAREPLANET_PLUGIN_BASENAME === $file ) {
			$row_meta = array(
				'docs' => '<a href="https://plugin-documentation.weareplanet.com/weareplanet/woocommerce/3.3.21/docs/en/documentation.html" aria-label="' . esc_html__( 'View Documentation', 'woo-weareplanet' ) . '">' . esc_html__( 'Documentation', 'woo-weareplanet' ) . '</a>',
			);
	...
	public static function update_1_0_0_initialize() {
		...
		$table_method_config = $wpdb->prefix . 'weareplanet_method_config';
		...
		$table_token_info = $wpdb->prefix . 'weareplanet_token_info';
		...
		$table_completion_job = $wpdb->prefix . 'weareplanet_completion_job';
		...
		$table_void_job = $wpdb->prefix . 'weareplanet_void_job';
		...
		$table_refund_job = $wpdb->prefix . 'weareplanet_refund_job';
	...
	public static function update_1_0_3_image_domain() {
		...
		$table_transaction_info = $wpdb->prefix . 'weareplanet_transaction_info'; //phpcs:ignore
		$table_method_config = $wpdb->prefix . 'weareplanet_method_config'; //phpcs:ignore
	...
	public static function update_1_0_4_failure_msg_and_attribute() {
		...
		$table_transaction_info = $wpdb->prefix . 'weareplanet_transaction_info'; //phpcs:ignore
		$table_attribute_options = $wpdb->prefix . 'weareplanet_attribute_options'; //phpcs:ignore
	...
	public static function update_1_0_6_shorten_table_names() {
		...
		$table_prefix = self::WEAREPLANET_DEPRECATED_TABLE_PREFIX;
		$table_rename_map = array(
			"{$wpdb->prefix}{$table_prefix}woocommerce_weareplanet_attribute_options" => "{$wpdb->prefix}{$table_prefix}weareplanet_attribute_options",
			"{$wpdb->prefix}{$table_prefix}woocommerce_weareplanet_completion_job" => "{$wpdb->prefix}{$table_prefix}weareplanet_completion_job",
			"{$wpdb->prefix}{$table_prefix}woocommerce_weareplanet_method_configuration" => "{$wpdb->prefix}{$table_prefix}weareplanet_method_config",
			"{$wpdb->prefix}{$table_prefix}woocommerce_weareplanet_refund_job" => "{$wpdb->prefix}{$table_prefix}weareplanet_refund_job",
			"{$wpdb->prefix}{$table_prefix}woocommerce_weareplanet_token_info" => "{$wpdb->prefix}{$table_prefix}weareplanet_token_info",
			"{$wpdb->prefix}{$table_prefix}woocommerce_weareplanet_transaction_info" => "{$wpdb->prefix}{$table_prefix}weareplanet_transaction_info",
			"{$wpdb->prefix}{$table_prefix}woocommerce_weareplanet_void_job" => "{$wpdb->prefix}{$table_prefix}weareplanet_void_job",
		);
	...
	public static function update_1_0_7_migrate_plugin_name_and_tables() {
		...
		$table_prefix = self::WEAREPLANET_DEPRECATED_TABLE_PREFIX;
		$table_rename_map = array(
			"{$wpdb->prefix}{$table_prefix}weareplanet_attribute_options" => "{$wpdb->prefix}weareplanet_attribute_options",
			"{$wpdb->prefix}{$table_prefix}weareplanet_completion_job" => "{$wpdb->prefix}weareplanet_completion_job",
			"{$wpdb->prefix}{$table_prefix}weareplanet_method_config" => "{$wpdb->prefix}weareplanet_method_config",
			"{$wpdb->prefix}{$table_prefix}weareplanet_refund_job" => "{$wpdb->prefix}weareplanet_refund_job",
			"{$wpdb->prefix}{$table_prefix}weareplanet_token_info" => "{$wpdb->prefix}weareplanet_token_info",
			"{$wpdb->prefix}{$table_prefix}weareplanet_transaction_info" => "{$wpdb->prefix}weareplanet_transaction_info",
			"{$wpdb->prefix}{$table_prefix}weareplanet_void_job" => "{$wpdb->prefix}weareplanet_void_job",
		);
		self::rename_tables( $table_rename_map );
	}
			...
			add_action(
				...
				function () {
					...
					<div class="notice notice-error">
						<p><?php __( 'WooCommerce is not activated. Please activate WooCommerce to use the payment integration.', 'woo-weareplanet' ); ?></p>
					</div>
					<?php
				}
			);
		...
		if ( version_compare( $woocommerce_data['Version'], WC_WEAREPLANET_REQUIRED_WC_MAXIMUM_VERSION, '>' ) ) {
			...
			if ( ! WC_Admin_Notices::user_has_dismissed_notice( $notice_id ) ) {
				$message = sprintf(
					...
					__( 'The plugin WeArePlanet has been tested up to WooCommerce %1$s but you have installed the version %2$s. Please notice that this is not recommended.', 'woo-weareplanet' ),
					WC_WEAREPLANET_REQUIRED_WC_MAXIMUM_VERSION,
	...
	public static function update_1_0_8_store_default_status_mappings() {
		$status_adapter = new WC_WeArePlanet_Order_Status_Adapter();
