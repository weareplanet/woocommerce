...
class WC_WeArePlanet_Order_Status_Adapter
{
	...
	public function add_order_statuses( $order_statuses ) {
		...
		foreach ( $results as $row ) {
			...
			$order_statuses[ $row['option_value'] ] = sprintf(
				...
				_x( '%s', 'Order status label', 'woo-weareplanet' ),
				$status_label
			);
		}
	...
	{
		$is_custom_mapping_enabled = WC_WeArePlanet_Helper::is_custom_status_mapping_enabled();
		...
		foreach ( $default_mappings as $transaction_status => $default_order_status ) {
			$option_key = self::WEAREPLANET_ORDER_STATUS_MAPPING_PREFIX . $transaction_status;
	...
	{
		return strpos( $order_status, 'wc-wearep-' ) === 0 || strpos( $order_status, 'wearep-' ) === 0;
	}
	 ...
	 * - weareplanet_order_status_mapping_completed -> wc-wearep-waiting -> OrderInternalStatus::ON_HOLD
	 ...
	 * - weareplanet_order_status_mapping_processing -> wc-processing -> OrderInternalStatus::PROCESSING
	 * - weareplanet_order_status_mapping_completed -> wc-processing -> OrderInternalStatus::PROCESSING
	 *
	...
	public function get_default_status_mappings() : array
	{
		$default_mappings = array(
			...
			self::WEAREPLANET_STATUS_COMPLETED => 'wc-on-hold',
		...
		if ( WC_WeArePlanet_Helper::is_custom_status_mapping_enabled() ) {
			$default_mappings[ self::WEAREPLANET_STATUS_COMPLETED ] = 'wc-wearep-waiting';
		}
	...
	public function update_order_status( WC_Order $order, ?string $status, ?string $default_status, string $note = '', bool $manual = false ): void
	{
		...
		$change_setting_enabled = 'yes' === get_option( WooCommerce_WeArePlanet::WEAREPLANET_CK_CHANGE_ORDER_STATUS );
		$is_virtual_order = WC_WeArePlanet_Helper::is_order_virtual( $order );
		$should_force_completed = (
			$change_setting_enabled
			&& $is_virtual_order
			&& null !== $status
			&& strtolower( (string) $status ) === strtolower( \WeArePlanet\Sdk\Model\TransactionState::FULFILL )
		);

		// Preserve WooCommerce's completed status only once the transaction reaches FULFILL.
		$new_status = $should_force_completed ? 'completed' : $this->map_weareplanet_status_to_woocommerce( $status );
	...
	public function get_order_status_on_payment_complete( string $status, int $order_id, WC_Order $order ): string
	{
		...
		$is_fulfilled = $transaction_info
			&& strtolower( $transaction_info->get_state() ) === strtolower( \WeArePlanet\Sdk\Model\TransactionState::FULFILL );
	 ...
	 *
	 * @since 1.0.0
	 * @param string|null $weareplanet_status The transaction status from WeArePlanet.
	...
	{
		if ( null === $weareplanet_status || '' === $weareplanet_status ) {
			return null;
		}

		$status = get_option( self::WEAREPLANET_ORDER_STATUS_MAPPING_PREFIX . strtolower( $weareplanet_status ), null );
		if ( ! WC_WeArePlanet_Helper::is_custom_status_mapping_enabled() && $status && $this->is_custom_order_status( $status ) ) {
			$default_mappings = $this->get_default_status_mappings();
			$status_key = strtolower( $weareplanet_status );
