...
if ( ! class_exists( 'WooCommerce_WeArePlanet' ) ) {

	/**
	 * Main WooCommerce WeArePlanet Class
	 *
	 * @class WooCommerce_WeArePlanet
	 */
	final class WooCommerce_WeArePlanet {
		...
		/**
		 * Main WooCommerce WeArePlanet Instance.
		 *
		 * Ensures only one instance of WooCommerce WeArePlanet is loaded or can be loaded.
		 *
		 * @return WooCommerce_WeArePlanet - Main instance.
		 */
		...
		/**
		 * WooCommerce WeArePlanet Constructor.
		 */
		...
		/**
		 * Define WC WeArePlanet Constants.
		 */
		protected function define_constants() {
			$this->define( 'WC_WEAREPLANET_PLUGIN_FILE', __FILE__ );
			$this->define( 'WC_WEAREPLANET_ABSPATH', __DIR__ . '/' );
			$this->define( 'WC_WEAREPLANET_PLUGIN_BASENAME', plugin_basename( __FILE__ ) );
			$this->define( 'WC_WEAREPLANET_VERSION', $this->version );
			$this->define( 'WC_WEAREPLANET_REQUIRED_PHP_VERSION', '7.4' );
			$this->define( 'WC_WEAREPLANET_REQUIRED_WP_VERSION', '6.0' );
			$this->define( 'WC_WEAREPLANET_REQUIRED_WC_VERSION', '8.0' );
			$this->define( 'WC_WEAREPLANET_REQUIRED_WC_MAXIMUM_VERSION', self::WC_MAXIMUM_VERSION );
			$this->define( 'WC_WEAREPLANET_REQUIRED_WC_SUBSCRIPTION_VERSION', self::REQUIRED_WC_SUBSCRIPTION_VERSION );
		}
		...
		protected function includes() {
			 ...
			 * Class autoloader.
			 */
			require_once WC_WEAREPLANET_ABSPATH . 'includes/class-wc-weareplanet-autoloader.php';
			require_once WC_WEAREPLANET_ABSPATH . 'weareplanet-sdk/autoload.php';

			require_once WC_WEAREPLANET_ABSPATH . 'includes/class-wc-weareplanet-migration.php';
			require_once WC_WEAREPLANET_ABSPATH . 'includes/class-wc-weareplanet-email.php';
			require_once WC_WEAREPLANET_ABSPATH . 'includes/class-wc-weareplanet-return-handler.php';
			require_once WC_WEAREPLANET_ABSPATH . 'includes/class-wc-weareplanet-webhook-handler.php';
			require_once WC_WEAREPLANET_ABSPATH . 'includes/class-wc-weareplanet-unique-id.php';
			require_once WC_WEAREPLANET_ABSPATH . 'includes/class-wc-weareplanet-customer-document.php';
			...
			require_once WC_WEAREPLANET_ABSPATH . 'includes/packages/gift-card/class-wc-weareplanet-packages-gift-card.php';
		...
		protected function init_hooks() {
			...
			add_action(
				'wp_ajax_weareplanet_is_payment_method_available',
				array(
			...
			add_action(
				'wp_ajax_nopriv_weareplanet_is_payment_method_available',
				array(
					'WC_WeArePlanet_Blocks_Support',
					'is_payment_method_available',
				)
			...
			add_action(
				'wp_ajax_weareplanet_get_payment_methods',
				array(
					'WC_WeArePlanet_Blocks_Support',
			...
			add_action(
				'wp_ajax_nopriv_weareplanet_get_payment_methods',
				array(
					'WC_WeArePlanet_Blocks_Support',
					'get_payment_methods_json',
				)
			...
			add_action(
				'wp_ajax_weareplanet_custom_order_status_save_changes',
				array(
					$this,
			...
			add_action(
				'wp_ajax_weareplanet_custom_order_status_delete',
				array(
					$this,
		...
		public static function plugin_deactivate() {
			...
			$old_plugin_prefix = WC_WeArePlanet_Migration::WEAREPLANET_DEPRECATED_PLUGIN_PREFIX;
			$plugin_current_version = self::get_installed_plugin_version( $old_plugin_prefix . 'weareplanet' ); // The slug of the old plugin.

			// Check if the plugin version is lower than 3.1.0.
			if ( version_compare( $plugin_current_version, self::WEAREPLANET_UPGRADE_VERSION, '<' ) ) {
		...
		public function secure_redirect_order_confirmed( $order_id ) {
			...
			$wc_service_transaction = WC_WeArePlanet_Service_Transaction::instance();
			$sdk_service_transaction = new \WeArePlanet\Sdk\Service\TransactionService( WC_WeArePlanet_Helper::instance()->get_api_client() );
			$wc_transaction_info = WC_WeArePlanet_Entity_Transaction_Info::load_by_order_id( $order_id );

			if ( property_exists( $wc_transaction_info, 'get_transaction_id' ) ) {
				$state = $sdk_service_transaction->read( get_option( self::WEAREPLANET_CK_SPACE_ID ), $wc_transaction_info->get_transaction_id() )->getState();
				if ( \WeArePlanet\Sdk\Model\TransactionState::CONFIRMED === $state ) {
					wp_redirect($wc_service_transaction->get_payment_page_url(get_option(self::WEAREPLANET_CK_SPACE_ID), $wc_transaction_info->get_transaction_id())); //phpcs:ignore
					exit;
				}
			}
		}
		...
		public static function migrate_plugin_data_on_activation() {
			$old_option_prefix = WC_WeArePlanet_Migration::WEAREPLANET_DEPRECATED_TABLE_PREFIX;
			$old_plugin_prefix = WC_WeArePlanet_Migration::WEAREPLANET_DEPRECATED_PLUGIN_PREFIX;

			$plugin_slug = $old_plugin_prefix . 'weareplanet'; // The slug of the old plugin.
			 ...
			 */
			if ( version_compare( $installed_version, self::WEAREPLANET_UPGRADE_VERSION, '>=' ) ) {
				return;
			}

			global $wpdb;
			$options_to_migrate = [
				$old_option_prefix . WC_WeArePlanet_Migration::WEAREPLANET_CK_DB_VERSION,
				$old_option_prefix . WC_WeArePlanet_Service_Manual_Task::WEAREPLANET_CONFIG_KEY,
				$old_option_prefix . self::WEAREPLANET_CK_SPACE_ID,
				$old_option_prefix . self::WEAREPLANET_CK_SPACE_VIEW_ID,
				$old_option_prefix . self::WEAREPLANET_CK_APP_USER_ID,
				$old_option_prefix . self::WEAREPLANET_CK_APP_USER_KEY,
				$old_option_prefix . self::WEAREPLANET_CK_CUSTOMER_INVOICE,
				$old_option_prefix . self::WEAREPLANET_CK_CUSTOMER_PACKING,
				$old_option_prefix . self::WEAREPLANET_CK_SHOP_EMAIL,
				...
				$old_option_prefix . self::WEAREPLANET_CK_CHANGE_ORDER_STATUS,
			...
			foreach ( $options_to_migrate as $option_name ) {
				...
				if ( false !== $option_value ) {
					// Rename the options to the new prefix 'weareplanet_'.
					$new_option_name = str_replace( $old_option_prefix . 'weareplanet_', 'weareplanet_', $option_name );
		 ...
		 *
		 * Locales found in:
		 * - WP_LANG_DIR/woo-weareplanet/woo-weareplanet-LOCALE.mo
		 */
		...
		public function loaded() {
			...
			add_filter(
			 	...
			 	function($value, $order) {
					...
					if (substr_count($order_data['payment_method'], "weareplanet")) {
		...
		public function maybe_boot_subscriptions() {
			...
			require_once WC_WEAREPLANET_ABSPATH . 'includes/class-wc-weareplanet-subscription.php';
			require_once WC_WEAREPLANET_ABSPATH . 'includes/class-wc-weareplanet-subscription-gateway.php';
			if ( class_exists( 'WC_WeArePlanet_Subscription' ) ) {
				WC_WeArePlanet_Subscription::instance()->boot();
			}
		}
		...
		public static function is_subscriptions_compatible(): bool  {
			...
			if ( version_compare( $woocommerce_subscriptions_data['Version'], WC_WEAREPLANET_REQUIRED_WC_SUBSCRIPTION_VERSION, '<' ) ) {
				return false;
			}
			return true;
		}
		...
		public function after_remove_product_from_cart( $removed_cart_item_key, $cart ) {
			...
			if ( $this->is_product_type_subscription( $product ) ) {
				...
				$service_transaction = WC_WeArePlanet_Service_Transaction::instance();
	...
	public function add_order_statuses( $order_statuses ) {
		$order_statuses['wc-wearep-redirected'] = _x( 'Redirected', 'Order status', 'woo-weareplanet' );
		$order_statuses['wc-wearep-waiting'] = _x( 'Waiting', 'Order status', 'woo-weareplanet' );
		$order_statuses['wc-wearep-manual'] = _x( 'Manual Decision', 'Order status', 'woo-weareplanet' );
	...
	public function valid_order_statuses_for_payment( $statuses, $order = null ) { //phpcs:ignore
		...
		public function custom_order_status_save_changes() {
			...
			if ( ! isset( $_POST['weareplanet_order_statuses_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['weareplanet_order_statuses_nonce'] ) ), 'weareplanet_order_statuses_nonce' ) ) {  //phpcs:ignore
			...
			if ( isset( $data['key'] ) ) {
				...
				wp_send_json_success([
					...
					'order_status' => [
						...
						'type'  => __( 'custom' , 'woo-weareplanet' )
					]
				]);
			} else {
		...
		public function custom_order_status_delete() {
			...
			// Verify nonce.
			if ( ! isset( $_POST['weareplanet_order_statuses_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['weareplanet_order_statuses_nonce'] ) ), 'weareplanet_order_statuses_nonce' ) ) {
				wp_send_json_error( 'Invalid request: missing nonce value', 403 );
			}
			...
			// Check if this order status is being used in wp_options with key prefix "weareplanet_order_status_mapping_".
			$like_pattern = WC_WeArePlanet_Order_Status_Adapter::WEAREPLANET_ORDER_STATUS_MAPPING_PREFIX . '%';
			...
			if ( $is_used > 0 ) {
				...
				wp_send_json_error( ['message' => __( 'Cannot delete: this status is currently in use.', 'woo-weareplanet' ), 'key' => ucfirst( $sanitized_key )], 400 );
			}

			 // Get the custom order status in wp_options with key prefix "weareplanet_order_status_mapping_".
			$like_pattern = WC_WeArePlanet_Order_Status_Adapter::WEAREPLANET_CUSTOM_ORDER_STATUS_PREFIX . '%';
			...
			wp_send_json_success( ['message' => __( 'Deleted successfully', 'woo-weareplanet' ), 'key' => ucfirst( $sanitized_key )] );
		}
		...
		public function set_js_async( $tag, $handle, $src ) { //phpcs:ignore
			$async_script_handles = array( 'weareplanet-device-id-js' );
		...
		public function enqueue_javascript_script() {
			if ( is_cart() || is_checkout() ) {
				$unique_id = isset( $_COOKIE['wc_weareplanet_device_id'] ) ? sanitize_text_field( wp_unslash( $_COOKIE['wc_weareplanet_device_id'] ) ) : null;
				$space_id = get_option( self::WEAREPLANET_CK_SPACE_ID );
				$script_url = WC_WeArePlanet_Helper::instance()->get_base_gateway_url() . 's/' .
						$space_id . '/payment/device.js?sessionIdentifier=' .
						$unique_id;
				wp_enqueue_script( 'weareplanet-device-id-js', $script_url, array(), $this->get_version(), true );
			}
		}
		...
		public function enqueue_stylesheets() {
			if ( is_checkout() ) {
				wp_enqueue_style( 'weareplanet-checkout-css', $this->plugin_url() . '/assets/css/checkout.css', array(), $this->get_version() );
			}
		}
		...
		public function before_calculate_totals( $cart ) { //phpcs:ignore
			$GLOBALS['weareplanet_calculating'] = true;
		}
		...
		public function after_calculate_totals( $cart ) { //phpcs:ignore
			unset( $GLOBALS['weareplanet_calculating'] );
		}
		...
		public function add_gateways( $methods ) {
			...
			$method_configurations = WC_WeArePlanet_Entity_Method_Configuration::load_by_states_and_space_id(
				$space_id,
				array(
					WC_WeArePlanet_Entity_Method_Configuration::WEAREPLANET_STATE_ACTIVE,
					WC_WeArePlanet_Entity_Method_Configuration::WEAREPLANET_STATE_INACTIVE,
				)
			);
			try {
		...
		public function update_additional_customer_data( $arguments ) {
			...
			$custom_billing_date_of_birth_field_name = apply_filters( 'wc_weareplanet_billing_date_of_birth_field_name', '' );
			...
			$billing_gender = '';
			$custom_billing_gender_field_name = apply_filters( 'wc_weareplanet_billing_gender_field_name', '' );
			...
			if ( ! empty( $billing_date_of_birth ) ) {
				WC()->customer->add_meta_data( '_weareplanet_billing_date_of_birth', $billing_date_of_birth, true );
			}
			if ( ! empty( $billing_gender ) ) {
				WC()->customer->add_meta_data( '_weareplanet_billing_gender', $billing_gender, true );
			}

			if ( ! empty( $post_data['ship_to_different_address'] ) && ! wc_ship_to_billing_address_only() ) {
				$shipping_date_of_birth = '';
				$custom_shipping_date_of_birth_field_name = apply_filters( 'wc_weareplanet_shipping_date_of_birth_field_name', '' );
				...
				$shipping_gender = '';
				$custom_shipping_gender_field_name = apply_filters( 'wc_weareplanet_shipping_gender_field_name', '' );
				...
				if ( ! empty( $shipping_date_of_birth ) ) {
					WC()->customer->add_meta_data( '_weareplanet_shipping_date_of_birth', $shipping_date_of_birth, true );
				}
				if ( ! empty( $shipping_gender ) ) {
					WC()->customer->add_meta_data( '_weareplanet_shipping_gender', $shipping_gender, true );
				}
			} else {
				if ( ! empty( $billing_date_of_birth ) ) {
					WC()->customer->add_meta_data( '_weareplanet_shipping_date_of_birth', $billing_date_of_birth, true );
				}
				if ( ! empty( $billing_gender ) ) {
					WC()->customer->add_meta_data( '_weareplanet_shipping_gender', $billing_gender, true );
				}
			}
		}
		...
		public function log( $message, $level = WC_Log_Levels::WARNING ) {
			...
			$this->logger->log(
				...
				$message,
				array(
					'source' => 'woo-weareplanet',
				)
			);

			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( 'Woocommerce WeArePlanet: ' . $message ); //phpcs:ignore
			}
		}
		...
		protected function update_attribute_options( $attribute_id, $send ) {
			$attribute_options = WC_WeArePlanet_Entity_Attribute_Options::load_by_attribute_id( $attribute_id );
		...
		public function woocommerce_attribute_added( $attribute_id, $data ) { //phpcs:ignore
			...
			$nonce = isset( $_POST['weareplanet_attribute_option_send_nonce'] )
				? sanitize_text_field( wp_unslash( $_POST['weareplanet_attribute_option_send_nonce'] ) )
				: '';
			if ( ! $nonce || ! wp_verify_nonce( $nonce, 'weareplanet_attribute_option_send' ) ) {
				return;
			}
			$send = ! empty( $_POST['weareplanet_attribute_option_send'] ) ? 1 : 0;
		...
		public function woocommerce_rest_insert_product_attribute( $attribute, $request, $create ) { //phpcs:ignore
			if ( isset( $request['weareplanet_attribute_option_send'] ) ) {
				if ( $request['weareplanet_attribute_option_send'] ) {
		...
		public function woocommerce_rest_prepare_product_attribute( $response, $item, $request ) {
			...
			if ( 'view' === $context || 'edit' === $context ) {
				...
				$attribute_options = WC_WeArePlanet_Entity_Attribute_Options::load_by_attribute_id( $item->attribute_id );
				$data['weareplanet_attribute_option_send'] = $attribute_options->get_id() > 0 && $attribute_options->get_send();
	...
	add_action( 'woocommerce_loaded', function () {
		WooCommerce_WeArePlanet::instance()->boot();
		add_action( 'woocommerce_blocks_loaded', 'WC_WeArePlanet_Blocks_Support' );
	});

	function WC_WeArePlanet_Blocks_Support() { //phpcs:ignore
		if ( class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' ) ) {
			require_once __DIR__ . '/includes/class-wc-weareplanet-blocks-support.php';

			add_action(
				'woocommerce_blocks_payment_method_type_registration',
				function ( Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry ) {
					$payment_method_registry->register( new WC_WeArePlanet_Blocks_Support() );
				}
			);
		}
	}
