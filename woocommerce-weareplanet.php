...
if ( ! class_exists( 'WooCommerce_WeArePlanet' ) ) {
	...
	final class WooCommerce_WeArePlanet {
		...
		protected function define_constants() {
			...
			$this->define( 'WC_WEAREPLANET_ABSPATH', dirname( __FILE__ ) . '/' );
		...
		protected function init_hooks() {
            ...
            add_action(
                ...
                ['WC_WeArePlanet_Blocks_Support', 'is_payment_method_available']
            );
            add_action(
            ...
            add_action(
                ...
                ['WC_WeArePlanet_Blocks_Support', 'get_payment_methods']
            );
            add_action(
			...
			add_action(
				...
				['WC_WeArePlanet_Blocks_Support', 'enqueue_portal_scripts']
			);
			add_action( 'woocommerce_rest_checkout_process_payment_with_context',
				['WC_WeArePlanet_Blocks_Support', 'process_payment'], 10, 2);
		}
		 ...
		 */
		 ...
		 public function secure_redirect_order_confirmed($order_id) {
			...
			if (property_exists($wc_transaction_info,'get_transaction_id')) {
			  ...
			  if ($state == \WeArePlanet\Sdk\Model\TransactionState::CONFIRMED) {
		...
		public function load_plugin_textdomain() {
			...
			load_plugin_textdomain( 'woo-weareplanet', false, plugin_basename( dirname( __FILE__ ) ) . '/languages' );
		}
		...
		public function register_order_statuses() {
			register_post_status(
				...
				array(
					...
					'label_count' => _n_noop( 'WeArePlanet Processing <span class="count">(%s)</span>', 'WeArePlanet Processing <span class="count">(%s)</span>' ),
				)
			);
			register_post_status(
				'wc-wearep-waiting',
		...
		public function add_gateways( $methods ) {
			...
			$method_configurations = WC_WeArePlanet_Entity_Method_Configuration::load_by_states_and_space_id(
				$space_id,
				array(
					WC_WeArePlanet_Entity_Method_Configuration::STATE_ACTIVE,
					WC_WeArePlanet_Entity_Method_Configuration::STATE_INACTIVE,
				)
			);
			try {
		...
		public function log( $message, $level = WC_Log_Levels::WARNING ) {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( 'Woocommerce WeArePlanet: ' . $message );
			}
		}
    ...
    function WC_WeArePlanet_Blocks_Support() {
        if ( class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' ) ) {
            require_once dirname( __FILE__ ) . '/includes/class-wc-weareplanet-blocks-support.php';
