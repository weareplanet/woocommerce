...
jQuery(
	function($) {
		...
		var wc_weareplanet_management = {

			init : function() {
				this.handle_refund_button();
				this.show_refund_states();
				$( '#woocommerce-order-items' ).off( 'click.woo-weareplanet' );
				$( '#woocommerce-order-items' ).on(
					'click.woo-weareplanet',
					'button.weareplanet-completion-show',
					{
						self : this
					},
				...
				$( '#woocommerce-order-items' ).on(
					'click.woo-weareplanet',
					'button.action-weareplanet-completion-cancel',
					{
						self : this
					},
				...
				$( '#woocommerce-order-items' ).on(
					'click.woo-weareplanet',
					'button.action-weareplanet-completion-execute',
					{
						self : this
					},
				...
				$( '#woocommerce-order-items' ).on(
					'click.woo-weareplanet',
					'button.action-weareplanet-void-cancel',
					{
						self : this
					},
					this.cancel_void
				...
				$( '#woocommerce-order-items' ).on(
					'click.woo-weareplanet',
					'button.action-weareplanet-void-execute',
					{
						self : this
					},
					this.execute_void
				...
				$( '#woocommerce-order-items' ).on(
					'click.woo-weareplanet',
					'button.weareplanet-update-order',
					{
						self : this
					},
					this.update_order
			...
			handle_refund_button : function() {
				if ($( 'span#weareplanet-remove-refund' ).length > 0) {
			...
			show_refund_states : function() {
				$( 'tbody#order_refunds div.weareplanet-refund-status' )
				.each(
					function() {
						...
						if (state == 'failure') {
							refund_thumb
							.addClass( 'weareplanet-refund-state-failue' );
						} else if (state == 'pending'
						|| state == 'sent'
						|| state == 'created') {
							refund_thumb
							.addClass( 'weareplanet-refund-state-pending' );
						}
					}
				);
			...
			show_completion : function(event) {
				...
				$( 'div.wc-order-add-item' ).children(
					'button.weareplanet-completion-button'
				).each(
					function(key, value) {
						$( value ).show();
			...
			update_taxes_for_line_items : function() {
				var initial_amount = $( this ).data( 'weareplanet-initial-amount' );
			...
			execute_completion : function() {
				...
				if (window.confirm( weareplanet_admin_js_params.i18n_do_completion )) {
					...
					var data = {
						action : 'woocommerce_weareplanet_execute_completion',
			...
			show_void : function() {
				...
				$( 'div.wc-order-add-item' ).children( 'button.weareplanet-void-button' )
				.each(
					function(key, value) {
						$( value ).show();
						$( 'div.refund-actions' ).prepend( $( value ) );
			...
			execute_void : function() {
				...
				if (window.confirm( weareplanet_admin_js_params.i18n_do_void )) {

					var data = {
						action : 'woocommerce_weareplanet_execute_void',
						order_id : woocommerce_admin_meta_boxes.post_id,
			...
			update_order : function() {
				...
				var data = {
					action : 'woocommerce_weareplanet_update_order',
					order_id : woocommerce_admin_meta_boxes.post_id,
					security : woocommerce_admin_meta_boxes.order_item_nonce
				};
		...
		wc_weareplanet_management.init();
