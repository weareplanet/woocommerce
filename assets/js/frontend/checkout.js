...
/* global jQuery document window weareplanet_js_params */

jQuery(
	function ($) {
		var wc_weareplanet_checkout = {
			payment_methods : {},
			integrations: {
				LIGHTBOX: 'lightbox',
				IFRAME: 'iframe',
			 ...
			 */
			info : function () {
				var versions = window.weareplanet_js_params.versions || {};
				var info = [
					{library: 'integration', version: window.weareplanet_js_params.integration},
					...
					{library: 'woo-weareplanet', version: versions.weareplanet || null},
				];
				console.table( info, [] );
			},
			...
			init : function () {
				// Payment methods.
				this.$checkout_form.off( 'click.woo-weareplanet' ).on(
					'click.woo-weareplanet',
				...
				this.$checkout_form.off( 'button#place_order' ).on(
					...
					{ self : this },
					function (event) {
						if (weareplanet_js_params.integration === window.wc_weareplanet_checkout.integrations.LIGHTBOX ) {
							return false;
						}
					}
				);
				...
				if ($( document.body ).hasClass( 'woocommerce-order-pay' )) {
					...
					this.$order_review.off( 'click.woo-weareplanet' ).on(
						'click.woo-weareplanet',
						'input[name="payment_method"]',
						{
							self : this
				...
				window.wc_weareplanet_checkout = this;
			},
			...
			payment_method_click : function (event) {
				...
				if ( weareplanet_js_params.integration === self.integrations.PAYMENT_PAGE ) {
					return;
				}
				...
				var configuration = $( "#weareplanet-method-configuration-" + current_method );
			...
			enable_place_order_button: function () {
				...
				this.$order_button.removeClass( 'weareplanet-disabled' );
			},

			disable_place_order_button: function () {
				this.$order_button.prop( 'disabled', true );
				this.$order_button.addClass( 'weareplanet-disabled' );
			},
			...
			is_supported_method : function (method_id) {
				if (!method_id || method_id === 'weareplanet_zero') {
					return false;
				}
				return method_id && (method_id.indexOf( 'weareplanet_' ) === 0);
			},
			...
			register_method : function (method_id, configuration_id, container_id) {
				...
				if (this.checkout_form_identifier === '#order_review') {
					this.$checkout_form.off( 'submit.weareplanet' )
					.on(
						'submit.weareplanet',
						function () {
					...
					this.$checkout_form.off( 'checkout_place_order_' + method_id + '.weareplanet' )
					.on(
						'checkout_place_order_' + method_id + '.weareplanet',
						function () {
							return self.process_submit( method_id );
						}
					);
			...
			process_submit : function (method_id) {
				...
				if ( ! has_full_address) {
					...
					this.submit_error( weareplanet_js_params.i18n_not_complete );
					return false;
				}
						...
						$.ajax(
							{
								...
								success: function ( result ) {
									...
									if (typeof result.weareplanet == 'undefined') {
			...
			process_order_created : function (data, textStatus, jqXHR) {
				...
				if (weareplanet_js_params.integration && weareplanet_js_params.integration === self.integrations.LIGHTBOX ) {
					...
					if ( ! has_full_address) {
						...
						self.submit_error( weareplanet_js_params.i18n_not_complete );
						return false;
					}
					...
					if (typeof data.weareplanet == 'undefined') {
						return false;
					}
